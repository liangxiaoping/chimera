...
class ChimeraObjectMetaclass(type):
    def __init__(cls, names, bases, dict_):
        make_class_properties(cls)


@six.add_metaclass(ChimeraObjectMetaclass)
class ChimeraObject(object):
    FIELDS = []
    ...
    def _obj_from_primitive(cls, primitive):
        ...
        for field, value in primitive['chimera_object.data'].items():
            if isinstance(value, dict) and 'chimera_object.name' in value:
                setattr(instance, field, ChimeraObject.from_primitive(value))
            else:
                setattr(instance, field, value)

        instance._obj_changes = set(primitive['chimera_object.changes'])
        instance._obj_original_values = \
            primitive['chimera_object.original_values']
    ...
    def to_primitive(self):
        ...
        serialized.
        NOTE: Currently all the chimera objects contain primitive types that
        ...
        for field in self.FIELDS:
            if self.obj_attr_is_set(field):
                if isinstance(getattr(self, field), ChimeraObject):
                    data[field] = getattr(self, field).to_primitive()
                else:
                    data[field] = getattr(self, field)

        return {
            'chimera_object.name': class_name,
            'chimera_object.data': data,
            'chimera_object.changes': sorted(self._obj_changes),
            'chimera_object.original_values': dict(self._obj_original_values)
        }
...
class DictObjectMixin(object):
    """
    Mixin to allow ChimeraObjects to behave like dictionaries
...
class ListObjectMixin(object):
    ...
    def _obj_from_primitive(cls, primitive):
        ...
        for field, value in primitive['chimera_object.data'].items():
            if field == 'objects':
                instance.objects = [ChimeraObject.from_primitive(v) for v in
                                    value]
            elif isinstance(value, dict) and 'chimera_object.name' in value:
                setattr(instance, field, ChimeraObject.from_primitive(value))
            else:
                setattr(instance, field, value)
    ...
    def to_primitive(self):
        ...
        for field in self.FIELDS:
            if self.obj_attr_is_set(field):
                ...
                elif isinstance(getattr(self, field), ChimeraObject):
                    data[field] = getattr(self, field).to_primitive()
                else:
                    data[field] = getattr(self, field)
        ...
        return {
            'chimera_object.name': class_name,
            'chimera_object.data': data,
            'chimera_object.changes': list(self._obj_changes),
            'chimera_object.original_values': dict(self._obj_original_values)
        }
